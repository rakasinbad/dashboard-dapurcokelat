select storage_transaksi.outlet as "Outlet", namaoutlet as "Nama Outlet", namakota as "Wilayah", storage_transaksi.tgl_trx as "Tanggal Trx", date_format(waktu, '%H:%i:%S') as "Waktu Trx", storage_transaksi.no_trx as "No. Trx", storage_transaksi.kode_promo as "Kode Promo", namapemesan as "Nama Customer", phone as "Phone", 
    storage_transaksi.remark as "Remark",
    kode as "SKU", nama_sku as "Nama SKU", 
kategori.kategori as "SKU Category",
	...
	harga as "Price", qty as "Qty", sub_total as "Gross Sales", diskon_sku as "Discount", sub_total - diskon_sku as "Nett Sales", channel as 'Channel', od.paid_date as "Tanggal Pembayaran", od.deldate as "Tanggal Kirim/Pickup", od.delcharge as "Ongkos Kirim", od.metode_kirim as "Metode Pengiriman", od.jumlah_armada as "Jumlah Armada"
    from (
	select tgl_trx, waktu, outlet, namaoutlet, kode, nama_sku, sub_total, qty, diskon_sku, coalesce(stc.channel, src.channel) as channel, kode_promo, namapemesan, tlptujuan, remark, namakota, no_trx, harga, phone from (
	select
	    ...
	    (
			case 
				...
				when storage_transaksi.`nomember` in ('1009134', '589021', '807227') then 'GRABFOOD' /*589021: Grab food, 807227: Grab for business */
