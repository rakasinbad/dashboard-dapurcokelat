select 
    ...
    channel as "Channel"
    from (
	select tgl_trx,outlet, bruto, diskon_rupiah, netto, no_trx, 
	CASE 
    	...
    	WHEN coalesce(stc.channel, src.channel) in ('ECOMMERCE - Tokopedia', 'ECOMMERCE - SHOPEE MALL','Tik Tok') then 'E-Commerce'
...
LEFT JOIN tempo.`customer` ON
   ...
   AND `storage_transaksi`.`nomember` = tempo.`customer`.`id`
    ...
    OR `storage_transaksi`.`outlet` != 'XT')
		AND `storage_transaksi`.`tgl_trx` BETWEEN date '2025-08-25' AND date '2025-08-31'
...
WHERE 1 = 1
AND (LOWER(`storage_transaksi`.`outlet`) LIKE '%dt%')
group by channel