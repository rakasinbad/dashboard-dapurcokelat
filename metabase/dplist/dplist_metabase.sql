WITH ConsolidatedDeposit AS (
  SELECT
    ...
    SUM(
      CASE
        WHEN waktu < '2026-06-29 00:00:00' THEN depositplus - depositmin
        ELSE 0
      END
    ) AS start,
    ...
    SUM(
      CASE
        WHEN waktu BETWEEN '2026-06-29 00:00:00'
        AND '2026-06-29 23:59:59' THEN depositplus
        ELSE 0
      END
    ) AS dpplus,
    SUM(
      CASE
        WHEN waktu BETWEEN '2026-06-29 00:00:00'
        AND '2026-06-29 23:59:59' THEN depositmin
        ELSE 0
      END
    ) AS dpmin,
...
SELECT
  ...
  cc,
  dpmin,
  (start + dpplus - dpmin) AS balance,
  -- 4. Clean Closing Balance calculation
  waktu
FROM
  ConsolidatedDeposit
WHERE
