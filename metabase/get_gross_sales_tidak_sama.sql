select
  data.bruto,
  sum(data.harga * data.qty) as sub_total,
  data.bruto - sum(data.harga * data.qty) as perbandingan,
  data.no_trx
from
  (
    SELECT
    ...
    FROM
        ...
        FROM
          (
            ...
            FROM
              ...
              left JOIN dci.storage_transaksi_detail ON storage_transaksi.no_trx = storage_transaksi_detail.no_trx
            ...
            FROM
              ...
              left JOIN tempo.storage_transaksi_detail ON storage_transaksi.no_trx = storage_transaksi_detail.no_trx
              JOIN master.outlet ON storage_transaksi.outlet = outlet.kodeoutlet
              JOIN master.kota ON outlet.kota = kota.id
              LEFT JOIN tempo.`customer` ON `storage_transaksi`.`nomember_app` = tempo.`customer`.`nomember`
              AND `storage_transaksi`.`nomember` = tempo.`customer`.`id`
            ...
            FROM
              ...
              left JOIN sistemsurabaya.storage_transaksi_detail ON storage_transaksi.no_trx = storage_transaksi_detail.no_trx
              JOIN master.outlet ON storage_transaksi.outlet = outlet.kodeoutlet
              JOIN master.kota ON outlet.kota = kota.id
              LEFT JOIN tempo.`customer` ON `storage_transaksi`.`nomember_app` = tempo.`customer`.`nomember`
              AND `storage_transaksi`.`nomember` = tempo.`customer`.`id`
      ...
      left JOIN master.kategori ON left(kode, 2) = kategori.no
    WHERE
      1 = 1
  ) as data
group by
