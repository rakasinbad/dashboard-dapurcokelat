-- https://dashboard.dapurcokelat.com/question/94-sales-data?periode=thismonth&outlet=&channel=&no_trx=
select
	no_trx as "No Trx",
	...
	sum(sub_total) / sum(qty) as "Price",
	sum(qty) as "Qty",
	sum(sub_total) as "Gross",
	sum(diskon_sku) as "Diskon",
	sum(sub_total) - sum(diskon_sku) as "Netto",
	channel as 'Channel',
	outlet
from
	(
		...
		from
			(
				select
					...
					sub_total,
					qty,
					sub_total / bruto * diskon_rupiah as "diskon_sku",
