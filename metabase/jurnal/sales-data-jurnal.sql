-- //  https: / / dashboard.dapurcokelat.com / question / 125 - sales - data - jurnal - w - o - tax - jurnal - format ? periode = thismonth & outlet = & channel = & no_trx = & parent_outlet = & nama_sku =
select 
	CONCAT(outlet.namaoutlet, ',', channel) as "*Customer", '' as "Email", '' as "BillingAddress", 
	'' as "ShippingAddress", DATE_FORMAT(tgl_trx, "%d/%m/%Y") as '*InvoiceDate', DATE_FORMAT(tgl_trx, "%d/%m/%Y") as '*DueDate', 
	'' as "ShippingDate", '' as 'ShipVia', '' as 'TrackingNo', '' as 'CustomerRefNo', 
	CONCAT(
        'INV/',
        outlet, '/',
        REPLACE(tgl_trx, '-', ''), '-', 
        (
            CASE 
                WHEN storage_transaksi.jenistrx IN ('4', '8') THEN
                    CASE 
	...
	'' as "Message", '' as "Memo", nama_sku as "*ProductName", '' as 'Description', sum(qty) as '*Quantity', 'Pcs' as '*Unit',
	sum(sub_total)/sum(qty) as '*UnitPrice', '' as 'ProductDiscountRate(%)', sum(diskon_sku) as 'InvoiceDiscount(value or %)',
	'PPN' as 'TaxName',
	'11%' as 'TaxRate(%)',
	'' as 'ShippingFee',
	...
	select
		...
		(storage_transaksi.diskon_rupiah * (sum(storage_transaksi_detail.qty) * storage_transaksi_detail.harga) / storage_transaksi.bruto)  * 100 / 111 as diskon_sku,
		...
		CASE WHEN parent = ''
    OR parent is null then outlet.kodeoutlet else parent END as parent_outlet,
	    (
			case 
   ...
   AND storage_transaksi_detail.`status` = 1
	...
	JOIN master.outlet on storage_transaksi.outlet = outlet.kodeoutlet AND outlet.`status_report` = 1
...
LEFT JOIN tempo.`customer` ON
	...
	where
		storage_transaksi.spoil = 0 AND storage_transaksi.compliment = 0 AND
		(`storage_transaksi`.`outlet` != 'XC'
			OR `storage_transaksi`.`outlet` != 'XT')
		AND `storage_transaksi`.`tgl_trx` BETWEEN date '2025-11-01' AND date '2025-11-30'
		and jenistrx in ('1','4','8')
		and storage_transaksi.`status` not in ('0', '')
		AND `storage_transaksi`.`outlet` in (SELECT
			kodeoutlet
			...
			(
    			...
    			and jenis_outlet in ('', '1', '2', '9')
    				and kodeoutlet not in ('XX', 'XB'))
    				 or kodeoutlet in (SELECT kodeoutlet FROM master.`outlet` WHERE jenis_outlet = '5' AND STATUS = '1'))

GROUP BY no_trx, storage_transaksi_detail.kode, harga
	union
	select
		storage_transaksi.outlet,
		storage_transaksi.tgl_trx,
		...
		tempo.`customer`.`customer_gender`,
		tempo.`customer`.`customer_dob`,
		CASE WHEN parent = '' then outlet.kodeoutlet else parent END as parent_outlet,
	    (
			case 
				when jenistrx in ('4','8') then
				    case 
	...
	JOIN tempo.storage_transaksi_detail on storage_transaksi.no_trx = storage_transaksi_detail.no_trx AND storage_transaksi_detail.status = 1
	JOIN master.produk on produk.kode = storage_transaksi_detail.kode
	JOIN master.outlet on storage_transaksi.outlet = outlet.kodeoutlet AND outlet.`status_report` = 1
	join master.kota on outlet.kota = kota.id
	LEFT JOIN tempo.`customer` ON
	...
	JOIN sistemsurabaya.storage_transaksi_detail on storage_transaksi.no_trx = storage_transaksi_detail.no_trx AND storage_transaksi_detail.status = 1
	JOIN master.produk on produk.kode = storage_transaksi_detail.kode
	JOIN master.outlet on storage_transaksi.outlet = outlet.kodeoutlet AND outlet.`status_report` = 1
	join master.kota on outlet.kota = kota.id
	LEFT JOIN tempo.`customer` ON
...
WHERE 1 = 1
AND parent_outlet = 'AS'
group by nama_sku, tgl_trx, channel
