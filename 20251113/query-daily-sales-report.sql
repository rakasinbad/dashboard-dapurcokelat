select sum(bruto) as "Total Gross Sales", sum(diskon_rupiah) as "Total Discount", sum(netto) as "Total Nett Sales", channel as "Channel"
    from (
	select src.no_trx, CAST(DATE_FORMAT(tgl_trx, "%Y-%m-01") as DATE) as tgl_trx, coalesce(stc.channel, src.channel) as channel, kodeoutlet  as outlet, bruto, diskon_rupiah, netto from (
    	select
...
LEFT JOIN tempo.`customer` ON
   ...
   AND `storage_transaksi`.`nomember` = tempo.`customer`.`id`
    ...
    OR `storage_transaksi`.`outlet` != 'XT')
    		AND `storage_transaksi`.`tgl_trx` BETWEEN date '2025-07-28' AND date '2025-07-31'
